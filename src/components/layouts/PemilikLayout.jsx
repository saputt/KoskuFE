...
const nav = [
  { to: ROUTES.PEMILIK.DASHBOARD, label: 'Beranda', icon: 'dashboard' },
  { to: ROUTES.PEMILIK.KAMAR, label: 'Manajemen Kamar', icon: 'grid' },
  { to: ROUTES.PEMILIK.PENYEWAAN, label: 'Penyewaan', icon: 'receipt', badge: 3 },
  { to: ROUTES.PEMILIK.TAGIHAN, label: 'Tagihan', icon: 'card', badge: 5 },
  { to: ROUTES.PEMILIK.PEMBAYARAN, label: 'Pembayaran', icon: 'check', badge: 2 },
  { to: ROUTES.PEMILIK.KELUHAN, label: 'Keluhan', icon: 'alert', badge: 1 },
  { to: ROUTES.PEMILIK.PROFIL, label: 'Profil', icon: 'user' },
];
  ...
  return (
    <div className="app">
      <aside className="sidebar">
        <Link to={ROUTES.PEMILIK.DASHBOARD} className="brand">
          <span className="brand-mark"><svg className="icon" width="17" height="17" viewBox="0 0 24 24"><path d={d.house}/></svg></span>
          KosKu
        </Link>
        <nav className="side-nav">
          {nav.map(item => {
            const active = pathname === item.to || (item.to !== ROUTES.PEMILIK.DASHBOARD && pathname.startsWith(item.to));
            return (
